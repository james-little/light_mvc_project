...
class ResourcePoolSocketServer extends SocketServer
{
    ...
    {
        return empty($this->_resource_config[$category]) ? [] : $this->_resource_config[$category];
    }
...
//         $connection_resource_list = $this->getResource('db', $params['type'], $db_config);
        ...
        $params['param'] = empty($params['param']) ? [] : $params['param'];
...
//         $pool_key = $this->getPoolKey('db', $params['type']);
    ...
    {
        $log_file = make_file(TMP_DIR . DIRECTORY_SEPARATOR . Application::getProjectName() . '/resource_pool');
        if (!preg_match('#^/#', $log_file)) {
            $log_file = TMP_DIR . DIRECTORY_SEPARATOR . RUNTIME_ENVI . "_resource_pool_{$log_file}";
        }
        return $log_file;
    }
}
