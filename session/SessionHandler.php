...
class SessionHandler {
    ...
    public function open($path, $name) {
        __add_info('session opened: ' . session_id(),
    ...
    public function close() {
        __add_info('session closed: ' . session_id(),
            InfoCollector::TYPE_LOGIC, InfoCollector::LEVEL_DEBUG);
        return true;
    }
    /**
    ...
    public function destroy($session_id) {
        __add_info('session destroyed: ' . session_id(),
            InfoCollector::TYPE_LOGIC, InfoCollector::LEVEL_DEBUG);
    ...
    public function write($session_id, $session_data) {
        __add_info('write session: ' . $session_id,
            InfoCollector::TYPE_LOGIC, InfoCollector::LEVEL_DEBUG);
    ...
    private function insertSessionDataToDb($session_id, $session_data, $uid = null) {
        ...
        if($uid && $uid > 0) $session_data_map['uid'] = $uid;
        $last_insert_id = $this->model->insert($session_data_map);
        __add_info('session insert: ' . $last_insert_id,
            InfoCollector::TYPE_LOGIC, InfoCollector::LEVEL_DEBUG);
    ...
    private function updateSessionDataToDb($session_id, $session_data, $uid = null) {
        ...
        __add_info('session updated: ' . $affected_rows,
            InfoCollector::TYPE_LOGIC, InfoCollector::LEVEL_DEBUG);
    ...
    private function updateOldSessionIdWithNew($old_session_id, $new_session_id) {
        ...
        $affected_rows = $this->model->update($session_data_map, $where_params);
        __add_info(
            sprintf('old session_id is replaced by the new one %s: [o]%s->[n]%s',
                $affected_rows ? 'succeed' : 'falied', $old_session_id, $new_session_id),
            InfoCollector::TYPE_LOGIC, InfoCollector::LEVEL_DEBUG);
        return $affected_rows;
    }
    /**
    ...
    private function countSessionDataFromDb($session_id) {
        if(empty($session_id) || empty($this->model)) return 0;
        return $this->model->queryCount('session_id',
            'delete_flag = 0 AND session_id = :session_id AND session_expires >= :now',
            array(':session_id' => $session_id, ':now' => time()));
    }
    /**
    ...
    private function writeSessionDataToCache($session_id, $session_data) {
        ...
        __add_info(sprintf('write session to cache: %s => %s', $session_id, var_export($session_data, true)),
            InfoCollector::TYPE_LOGIC, InfoCollector::LEVEL_DEBUG);
    }
    /**
    ...
    private function clearSessionDataCache($session_id) {
        if(empty($session_id) || empty($this->cache)) return ;
        $cache_key = $this->getSessionCacheKey($session_id);
        $this->cache->delete($cache_key);
        __add_info('delete session data from cache: ' . $session_id,
            InfoCollector::TYPE_LOGIC, InfoCollector::LEVEL_DEBUG);
    }
    /**
     * encode session data
