...
class ResourcePoolSocketServer extends SocketServer {
    ...
    protected function getDataFromDb($params) {
        ...
        $validate_result_list = Validator::getInstance()->validate(array(
            ...
            'table_name' => array('not empty', Validator::DATA_TYPE_STRING),
        ...
        $db_config = $this->getDbConfig($params['table_name']);
        if (empty($db_config)) {
        ...
        $resource_key = $this->_resource_pool->getResourceKey(get_class($command_exec->getDriver()), $db_config);
        $this->_resource_pool->registerResource('db', $resource_key, $connection);
        ...
        $result = $command_exec->$method_name($params['sql'], $params['param']);
        $pool_key = $this->getPoolKey($command, $params['type']);
