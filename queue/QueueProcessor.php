...
use lightmvc\appLog\model\AppLogDataModel;
use lightmvc\Communicator;
use lightmvc\ClassLoader;
use lightmvc\exception\ExceptionCode;
use lightmvc\exception\QueueProcessorException;
use lightmvc\log\writer\LogWriterStream;
use lightmvc\Monitor;
use lightmvc\queue\Queue;
use lightmvc\Url;
...
class QueueProcessor
{
    ...
    public function applyConfig($config)
    {
        if (!isset($config['communicator'])) {
            throw new QueueProcessorException(
                "no communicator config",
                ExceptionCode::QUEUEPROCESSOR_CONFIG_ERROR
            );
        }
    ...
    protected function _sendRequest($request_list)
    {
        ...
        while ($request_list_count) {
            ...
            if ($request['http_method'] == AppLogDataModel::REQUEST_TYPE_POST) {
                $is_by_post = true;
                $post_var   = empty($request['request_params']) ? [] : json_decode($request['request_params'], true);
            } elseif ($request['http_method'] == AppLogDataModel::REQUEST_TYPE_GET) {
                $request_param_list = empty($request['request_params']) ? [] : json_decode($request['request_params'], true);
    ...
    {
        $success_request_id_list = [];
        // create response parser
        $queue_processor_parser = ClassLoader::loadClass(
            '\lightmvc\appLog\logic\queue\processor\QueueProcessorParser'
        );
        $queue_processor_parser->addRule(
            '\lightmvc\appLog\logic\queue\processor\rule\QueueProcessorParserRule1'
        );
        $queue_processor_parser->addRule(
            '\lightmvc\appLog\logic\queue\processor\rule\QueueProcessorParserRule2'
        );
        $queue_processor_parser->addRule(
            '\lightmvc\appLog\logic\queue\processor\rule\QueueProcessorParserRule3'
        );
